  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <form onSubmit={handleEmailLogin} className="flex flex-col gap-4 animate-slide-up-spring delay-225">
        ...
        <Input label="Senha" type="password" autoComplete="current-password" placeholder="••••••••" icon={Lock} value={password} onChange={(e) => setPassword(e.target.value)} required />

        {error && (
          <div className="flex items-center gap-2.5 rounded-2xl px-4 py-3 text-sm animate-scale-in" style={{ backgroundColor: 'var(--color-error-bg)', color: 'var(--color-error)', border: '1px solid var(--color-error)' }}>
        ...
        >
          {loadingEmail ? <span className="flex items-center justify-center gap-2"><span className="h-4 w-4 rounded-full border-2 border-white border-t-transparent animate-spin" />Entrando…</span> : 'Entrar'}
        </button>
      </form>

      <p className="mt-6 text-center text-sm animate-slide-up-spring delay-300" style={{ color: 'var(--color-text-secondary)' }}>
        Não tem uma conta?{' '}
        <button type="button" onClick={() => openModal('signup')} className="font-bold transition-opacity hover:opacity-70" style={{ color: 'var(--color-primary)' }}>
          Cadastre-se grátis →
        </button>
      </p>
    </div>

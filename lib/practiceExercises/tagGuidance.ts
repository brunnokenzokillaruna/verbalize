...
export function buildTagGuidance(
  ...
  if (tag === 'VOC') {
    const types = pick(['context-choice', 'reverse-translation', 'sentence-builder', 'word-bank-translation']);
    return [
  ...
  if (tag === 'DIAL') {
    const types = pick(['social-roleplay', 'scrambled-conversation', 'interactive-subtitles', 'speak-repeat']);
    return [
      types.length
        ...
        : '',
      `- At least 1 exercise MUST be 'speak-repeat' — the learner must speak a line aloud from the mini-story.`,
      productionLine,
  ...
  if (tag === 'EXPR') {
    const types = pick(['social-roleplay', 'context-choice', 'sentence-builder', 'speak-repeat']);
    return [
      types.length
  ...
  if (tag === 'CULT') {
    const types = pick(['social-roleplay', 'context-choice', 'sentence-builder', 'logic-connectors', 'speak-repeat']);
    return [
